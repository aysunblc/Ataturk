...
/**
 * Final videosunu tam ekran oynatan bileşen
 */
const FinalVideoPlayer = ({ videoSrc, onEnded, onError }) => {
    return (
        <div style={{
            position: 'fixed',
            top: 0,
...
const Game = ({ selectedCharacter, onGameEnd, onRecordResult, onSaveUserAnswer, userAnswers = [], onClearUserAnswers }) => {
    ...
    const [showFinalVideo, setShowFinalVideo] = useState(false); // Final videosu gösteriliyor mu?
    ...
    /**
     * Debug: Track showFinalVideo state changes
     */
    useEffect(() => {
        console.log('🎬 showFinalVideo state changed:', showFinalVideo);
    }, [showFinalVideo]);
     ...
     * Performans optimizasyonu için bir sonraki olayın videolarını önceden yükler
     */
    useEffect(() => {
        ...
        if (currentEventIndex < events.length - 1) {
            ...
            // Son soruda final videosunu preload et
            const characterData = GAME_EVENTS[selectedCharacter.id];
            if (characterData && characterData.length > 0 && characterData[0].finalVideo) {
                const finalVideo = document.createElement('video');
                finalVideo.src = `/videos/${characterData[0].finalVideo}`;
    ...
    const handleVideoError = (error, videoType) => {
        ...
        if (videoType === 'intro') {
            ...
            console.warn('Final videosu yüklenemedi, analiz ekranına geçiliyor...');
            setShowFinalVideo(false);
            // Oyunu bitir - App.jsx'teki ANALYSIS state'ine geçiş yapacak
            onGameEnd();
        }
    };
    ...
    /**
     * AŞAMA 5: Sonraki Olaya Geçiş veya Oyunu Bitirme
     */
    const handleNextEvent = () => {
        console.log('🔄 handleNextEvent called');
        ...
        setTimeout(() => {
            if (currentEventIndex < events.length - 1) {
                // Sonraki soruya geç
                console.log('➡️ Sonraki soruya geçiliyor:', currentEventIndex + 1);
                setCurrentEventIndex(currentEventIndex + 1);
            } else {
                // Son soru - final videosuna geçiş yap
                console.log('🏁 Son soru tamamlandı, final videosuna geçiliyor');
                const characterData = GAME_EVENTS[selectedCharacter.id];
                console.log('🎬 Character data:', characterData);
                console.log('🎬 Final video:', characterData?.[0]?.finalVideo);
                
                if (characterData && characterData.length > 0 && characterData[0].finalVideo) {
                    console.log('✅ Final video bulundu, gösteriliyor');
                    setShowFinalVideo(true);
                } else {
                    // Final video yoksa direkt oyunu bitir
                    console.warn('⚠️ Final video tanımlı değil, oyun sonlandırılıyor.');
                    onGameEnd();
                }
            }
        }, 1000); 
    ...
    /**
     * Final videosu bittiğinde oyunu bitir (App.jsx'teki ANALYSIS state'ine geçiş için)
     */
    const handleFinalVideoEnded = () => {
        console.log('🎬 Final video ended');
        console.log('📊 Oyun bitiyor, App.jsx analiz ekranına geçecek');
        
        // Oyunu bitir - App.jsx'teki handleFinalVideoEnd çağrılacak
        onGameEnd();
    };
    ...
    if (!currentEvent) {
        return (
            <div id="game-container" className="game-wrapper" style={{ padding: '100px var(--spacing-sm)' }}>
                ...
                >
                    Ana Menüye Dön <span role="img" aria-label="ev">🏠</span>
                </button>
            </div>
        );
    }
    ...
    return (
        <div className="game-wrapper" style={{display: 'flex', flexDirection: 'column' }}>
            ...
            {/* FINAL VİDEOSU (Tüm sorular bitince) */}
            {showFinalVideo && (
                <FinalVideoPlayer
                    videoSrc={`/videos/${GAME_EVENTS[selectedCharacter.id][0].finalVideo}`}
                    onEnded={handleFinalVideoEnded}
                    onError={(e) => handleVideoError(e, 'final')}
                />
            )}
